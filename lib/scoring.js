...
export function scoreRaquettes(raquettes, quiz, topN = 10) {
  ...
  // Le #1 du classement principal
  const top1Id = scored[0]?.id

  // Joffrey = meilleure marge, mais jamais le #1 du classement principal
  const joffreyRaquette = scoredJoffrey.find(r => r.id !== top1Id)

  // Classement final : top sans Joffrey, insertion en 2e
  const top = scored.filter(r => r.id !== joffreyRaquette?.id).slice(0, topN)

  if (joffreyRaquette) {
    const joffreyAvecFlag = { ...scored.find(r => r.id === joffreyRaquette.id) || joffreyRaquette, isJoffrey: true }
    top.splice(1, 0, joffreyAvecFlag)
  }
