...
// Poids final explicite — modifiable facilement
const W_TECH     = 0.60
const W_MARGE    = 0.15
const W_STOCK    = 0.15
...
function getScoringPoids(sensations) {
  ...
  sensations.forEach((s, i) => {
    ...
    DIMS.forEach(d => {
      combined[d] += poids[d] * SENSATION_WEIGHTS[i]
    })
  })
  return combined
}
...
function couche2(pool) {
  ...
  return pool.map(r => {
    ...
    const scoreFinal =
      (r.scoreTech * r.coeffGenre * W_TECH) +
      (scoreMarge    * W_MARGE) +
      (scoreStock    * W_STOCK) +
      (scoreRotation * W_ROTATION)

    return { ...r, scoreMarge, scoreStock, scoreRotation, scoreFinal: Math.round(scoreFinal) }
  })
}
