...
async function shopifyQuery(query, variables) {
  ...
  console.log('Shopify call:', endpoint)
...
export async function getRaquettes() {
  const domain = process.env.SHOPIFY_STORE_DOMAIN
  const query = 'query getRaquettes($cursor: String) { products(first: 250, after: $cursor, query: "product_type:Raquettes status:active") { pageInfo { hasNextPage endCursor } edges { node { id title handle images(first: 1) { edges { node { url altText } } } variants(first: 1) { edges { node { id price availableForSale inventoryQuantity inventoryItem { unitCost { amount } } } } } metafields(namespace: "custom", first: 20) { edges { node { key value } } } } } } }'
  let allRaquettes = []
  let cursor = null
...
function normalizeProduct(product, domain) {
  ...
  return {
    ...
    price, cost, marge,
    stock: variant.availableForSale ? (variant.inventoryQuantity || 1) : 0,
