...
// Cache mémoire du catalogue (valide 5 minutes dans la même instance serverless)
let _catalogueCache = null
let _catalogueCacheTime = 0
const CATALOGUE_TTL = 5 * 60 * 1000 // 5 minutes

export async function getRaquettes() {
  const now = Date.now()
  if (_catalogueCache && now - _catalogueCacheTime < CATALOGUE_TTL) {
  ...
  const result = allRaquettes.map(p => normalizeProduct(p, domain))
  _catalogueCache = result
  _catalogueCacheTime = Date.now()
  return result
}
