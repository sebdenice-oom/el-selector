...
export async function getRaquettes() {
  const query = 'query getRaquettes($cursor: String) { products(first: 250, after: $cursor, query: "product_type:Raquette") { pageInfo { hasNextPage endCursor } edges { node { id title handle images(first: 1) { edges { node { url altText } } } variants(first: 1) { edges { node { id price inventoryQuantity inventoryItem { unitCost { amount } } } } } metafields(namespace: "custom", first: 10) { edges { node { key value } } } } } } }'
...
export async function getVentesParProduit() {
  const query = 'query { shopifyqlQuery(query: "FROM sales SELECT product_id, SUM(net_quantity) AS units_sold WHERE product_type = \'Raquette\' SINCE -90d UNTIL today GROUP BY product_id") { ... on TableResponse { tableData { rowData columns { name dataType } } } } }'
