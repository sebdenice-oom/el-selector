...
function preFiltrerUpgrade(raquettes, raquetteExclueId, prixReference) {
  // Pré-filtre rapide : stock, schéma, prix dans une fourchette large (±60€)
  return raquettes.filter(r => {
    if (r.id === raquetteExclueId) return false
    if (r.stock <= 0 && !r.precommande) return false
    if (!r.schema || Object.keys(r.schema).length === 0) return false
    // Fourchette prix large (±60€ sur prix non réduit)
    if (prixReference > 0) {
      const prixNonReduit = r.compareAtPrice && r.compareAtPrice > r.price ? r.compareAtPrice : r.price
      if (Math.abs(prixNonReduit - prixReference) > 120) return false
    }
    return true
  })
}
...
export function scoreRaquettesUpgrade(raquettes, cibles, ciblesInitiales, prixReference, raquetteExclueId = null, topN = 12) {
  ...
  const candidats = preFiltrerUpgrade(raquettes, raquetteExclueId, prixReference)
  ...
  // Le #1 du classement principal ne peut pas être Joffrey
  const top1Id = scored[0]?.id
  const joffreyRaquette = scoredJoffrey.find(r => r.id !== top1Id)

  const top = scored.filter(r => r.id !== joffreyRaquette?.id).slice(0, topN)

  if (joffreyRaquette) {
    const base = scored.find(r => r.id === joffreyRaquette.id) || joffreyRaquette
    top.splice(1, 0, { ...base, isJoffrey: true })
  }
